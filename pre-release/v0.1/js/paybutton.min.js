...
var successDisplay=document.getElementById('modal-content');successDisplay.innerHTML='<div>'+
'<div>'+
...
var qrParams={ecclevel:'Q',fillcolor:'#FFFFFF',textcolor:'#000000',margin:'0.5',modulesize:12};if(document.implementation.hasFeature('http://www.w3.org/2000/svg','1.1')){genQR=QRCode.generateSVG(pbAttr.URI,qrParams);var XMLS=new XMLSerializer();genQR=XMLS.serializeToString(genQR);genQR='data:image/svg+xml;base64,'+
window.btoa(unescape(encodeURIComponent(genQR)));qrImage=genQR;}else{genQR=QRCode.generatePNG(pbAttr.URI,qrParams);qrImage=genQR;}
var pbContent='<div>'+
'<div>'+
'<div>'+
...
'</div>'+
'</div>'+
'</div>';var pbModal=new Modal({content:pbContent});pbModal.open();}
...
' = '+
pbAttr.bchAmount+
' BCH';openModal(pbAttr);}else{console.log('Error Price Not Found');}}else{console.log('Found Server But There Is An Error');}};fiatRequest.onerror=function(){console.log('Could Not Connect To Server');};fiatRequest.send();}
function mouseEnter(){var buttonText2=this.getAttribute('button-text-2')||'';buttonText2=buttonText2.trim();if(!buttonText2){var showAmount=this.getAttribute('amount')||'';showAmount=Number(showAmount.trim());var showType=this.getAttribute('amount-type')||'';showType=showType.trim().toUpperCase();buttonText2=showAmount+' '+showType;if(!showAmount||!showType){buttonText2='Click to send BCH';}}
...
this.addEventListener('mouseleave',buttonDefaultText,false);if('ontouchend'in window)
this.addEventListener('touchend',buttonDefaultText,false);}
function buttonDefaultText(){var buttonText=this.getAttribute('button-text')||'';buttonText=buttonText.trim();if(!buttonText){var showAmount=this.getAttribute('amount')||'';showAmount=Number(showAmount.trim());var showType=this.getAttribute('amount-type')||'';showType=showType.trim().toUpperCase();buttonText='Send Bitcoin Cash';if(!showAmount||!showType){buttonText='Send Bitcoin Cash';}}
this.innerHTML='<span>&nbsp</span>';var x=this;setTimeout(function(){x.innerHTML='<span>'+buttonText+'</span>';},200);}
document.addEventListener('DOMContentLoaded',function(){var payButton=document.getElementsByClassName('pay-button');for(var i=0;i<payButton.length;i++){var payButtons=payButton[i];window.addEventListener('load',buttonDefaultText.bind(payButtons),false);if(!('ontouchstart'in window))
payButtons.addEventListener('mouseenter',mouseEnter,false);if('ontouchstart'in window)
...
var Paybutton={render:function(id,config){var elem=document.getElementById(id);var newBtn=document.createElement('button');newBtn.setAttribute('id',id);if(config.style){newBtn.setAttribute('class','pay-button '+config.style);}else{newBtn.setAttribute('class','pay-button');};if(config.button_text){newBtn.setAttribute('button-text',config.button_text);};if(config.button_text_2){newBtn.setAttribute('button-text-2',config.button_text_2);};if(config.success_msg){newBtn.setAttribute('success-msg',config.success_msg);};if(config.address){newBtn.setAttribute('address',config.address);};if(config.amount){newBtn.setAttribute('amount',config.amount);};if(config.amount_type){newBtn.setAttribute('amount-type',config.amount_type);};if(config.paywall_field){newBtn.setAttribute('paywall-field',config.paywall_field);};if(config.success_callback){newBtn.setAttribute('success-callback',config.success_callback);}
