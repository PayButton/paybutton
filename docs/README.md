...
OP_RETURN is a [script](https://wiki.bitcoinsv.io/index.php/OP_RETURN) opcode that allows adding extra information to transactions beyond standard inputs and outputs. This can be useful for applications that want to include:
...
OpReturn is composed by three mandatory components:

1.  **Version**: A byte allowing future updates (currently always `0`).

2.  **Data Payload**: Custom information in UTF-8 format with the indicated size (can be empty).

3.  **Nonce**: Eight random bytes to differentiate payments (can be empty).

If the **data payload** or **nonce** is empty, it is represented by the `OP_0` [code](https://wiki.bitcoinsv.io/index.php/Opcodes_used_in_Bitcoin_Script).


### General OpReturn structure

    <OP_RETURN hex> <pushdata for protocol identifier> <protocol identifier> <version> <data> <nonce>

  
### How send data using OP_RETURN opcode in Paybutton
...
### Paybutton OpReturn encode examples:
    ...
    6a <pushdata for protocol identifier> 50415900 <version> <data> <paymentId>
...
-  `04` → 4 bytes hex value indicating size of data protocol identifier

-  `50415900` → PayButton identifier (ASCII `PAY` + `0x00`)

-  `00` → Version 0 pushed as `OP_0`

-  `0c` → Hex value indicating 12 bytes of data

-  `0102030405060708090a0b0c` → Data payload

-  `00` → No payment ID (nonce)
...
-  `04` → 4 bytes hex value indicating size of data protocol identifier `50415900`

-  `50415900` → PayButton identifier (ASCII `PAY` + `0x00`)

-  `00` → Version 0

-  `0c` → Hex indicating 12 bytes of data
